...
class AutoCorrelation(nn.Module):
    ...
    def time_delay_agg_full(self, values, corr):
        ...
        This version selects delays independently for each batch/head/channel.
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        if self.training:
            V = self.time_delay_agg_training(values, corr).permute(0, 3, 1, 2)
        else:
            V = self.time_delay_agg_inference(values, corr).permute(0, 3, 1, 2)
